---
title: "Create Nitrogen-Balance Validation Dataset from Public Sources"
author: "Kenneth Qin"
...
Identify counties with over `r THRESHOLD_CORNSOY` of cultivated acres consisting of corn or soy.

First, what proportion of counties have over `r THRESHOLD_CORNSOY` of their cultivated acreage consist of corn or soy?
...
Aside from corn and soybean, these counties also grow a substantial amount of winter wheat. Winter wheat demands almost twice as much N as corn, which may bias the corn N-balance downwards if total N applications are multiplied by the proportion of corn/soybean acreage within each county. However, these crops only make up to `r 1-THRESHOLD_CORNSOY` of total acreage in these counties.
...
Specifically, we will look at corn fertilizer rates. This means that in the EPA NEI-derived fertilizer application estimates, we will only use counties where over `r THRESHOLD_CORNSOY` of acreage consists of corn/soybean.
...
`r nrow(Corn.Fertilizer.County.2014.EPA.trim)` out of `r nrow(Corn.Fertilizer.County.2014.EPA)` counties still remain in the trimmed N-balance dataset.
...
Assuming IPNI nutrient removal rate of **`r LBS_N_REMOVED_PER_BU_CORN` lbs. N / bu**, calculate county-scale N-balance (lbs. N / ac).
